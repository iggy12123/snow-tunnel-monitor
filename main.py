...
def build_web():
    ...
    <head>
        ...
        <style>
            ...
            .error {{ color: #ff4757; font-size: 0.8em; padding: 10px; border: 1px solid #ff4757; border-radius: 10px; }}
        </style>
    </head>
    <body>
        <h2 style="margin-bottom: 5px;">🚗 雪隧即時數據儀表板</h2>
        <p style="color:#aaa; font-size:0.8em;">最後更新時間：{time_str}</p>
    """
    
    if not token:
        html += f"<div class='error'>驗證異常：{auth_msg}</div>"
    elif not data:
        html += "<div class='error'>數據連線中，請稍後再試</div>"
    else:
